    ...
    if (this->response.pData != 0)
    {
        ...
        if (root != NULL)
        {
            ...
            if (stonesArray != NULL)
            {
                ...
                {
                    cJSON *color = cJSON_GetArrayItem(stonesArray, stonesIndex);
                    char * colorStr = color->valuestring;
                    if (strcmp(colorStr, "Red") == 0)
                    {
                        tmpHeartBeatResponse.status.red = 1;
                    }
                    else if (strcmp(colorStr, "Green") == 0)
                    {
                        tmpHeartBeatResponse.status.green = 1;
                    }
                    else if (strcmp(colorStr, "Yellow") == 0)
                    {
                        tmpHeartBeatResponse.status.yellow = 1;
                    }
                    else if (strcmp(colorStr, "Magenta") == 0)
                    {
                        tmpHeartBeatResponse.status.magenta = 1;
                    }
                    else if (strcmp(colorStr, "Blue") == 0)
                    {
                        tmpHeartBeatResponse.status.blue = 1;
                    }
                    else if (strcmp(colorStr, "Cyan") == 0)
                    {
                        tmpHeartBeatResponse.status.cyan = 1;
                    }
                    else
                    {
                        ESP_LOGE(TAG, "Unknown color: %s", colorStr);
                    }
                }
            }
            // else
            // {
                // ESP_LOGE(TAG, "No stones array found");
            // }
            ...
            if (event != NULL)
            {
                ...
                if (stoneColor != NULL)
                {
                    ...
                    {
                        ESP_LOGI(TAG, "Unknown color: %s", colorStr);
                    }
                }
                // else
                // {
            ...
            if (timestamp != NULL)
            {
                struct timeval newTime = { .tv_sec = (time_t)timestamp->valueint, .tv_usec = 0 };
        ...
        {
            ESP_LOGE(TAG, "JSON parse failed. json = \"%s\"", (char *)this->response.pData);
        }
    }
    else
    {
            ...
            if(err == ESP_OK)
            {
                ...
                switch(pCurr->request.requestType)
                {
                    ...
                    case HTTPGAMECLIENT_HTTPREQUEST_HEARTBEAT:
                        ...
                        if (_HTTPGameClient_ParseJsonResponse(this) == ESP_OK)
                        {
...
static void HTTPGameClient_GameStateRequestNotificationHandler(void *pObj, esp_event_base_t eventBase, int notificationEvent, void *notificationData)
{
    ...
    bool first = true;
    int offset = 0;
    for (int i = 0; i < MIN(pRequest->numPeerReports, MAX_PEER_MAP_DEPTH); i++)
    {
        if (first)
        {
            first = false;
